...
* [x] 型定義の実装（`NenrinConfig`, `Domain`, `Event`, `NenrinInput`）
* [x] Ridge 関連型（`Ridge`, `PolarAnchor`）
* [x] Geometry 関連型（`PolarPoint`, `XyPoint`, `RidgePolylinePolar`, `RidgePolylineXy`）
* [x] アルゴリズム注入型（`CurveAlgorithm`, `GeometryOptions`）
...
* `@nenrin/types` は type-only パッケージとして完成している
* ランタイムコードは含まず, 他パッケージの型インターフェースのみを提供
* 他パッケージは必要に応じて `@nenrin/types` から型を再エクスポート可能
...
1. **Phase 1 を開始**: `@nenrin/core` の実装に着手
    * まずは入力バリデーションから実装
    * 次に domain 正規化と角度処理
    * その後イベント集計と ridge 生成
2. Phase 1 完了後, Phase 2 (Geometry) へ移行
3. Phase 2 完了後, Phase 3 (Dots) へ移行
4. すべての Phase 完了後, Phase 4 でリリース準備
